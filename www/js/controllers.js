  ...
  var getuserdata=function(){
    ...
    .success(function(response) {
      ...
      hide();
      if(localStorage.getItem("businessType") == "service"){
        $state.go('service');
      }
      else{
        $state.go('home');
  ...
  var getprofile=function(){
    ...
    .success(function(response) {
      ...
      localStorage.setItem('businessType',response.results[0].type);
      getuserdata();
    })
  }
  var show = function() {
  ...
  var showpassword = function() {
   ...
   alertPopup.then(function(res) {
    ...
    //signup controller
    .controller('signup',function($scope, $http, $state, Restangular, $ionicPopup, $ionicLoading, $rootScope){

      $scope.signup = function (usernam, Fullname, business, Location, passwor, confirm, Type)
      {
        if (Fullname == null){
        ...
        if(Type == "retail"){
          ...
          localStorage.setItem('businessType',"service");
          businessType = "service";
        }

        show();
        var baseAccounts = Restangular.all('1/users');
        var newAccount = {username: usernam, fullname:Fullname, Business:business, location:Location, password:passwor, type:businessType};
        baseAccounts.post(newAccount);
        
        var baseObjects = Restangular.all('1/classes/In_Stock');
      //creates new object in parse with empty store details json
      var startobject = {sold:[], stock:[], spent:[], Account: 0 , deposit: [], bank:"", loan: 0 , payments:[], institute:""};
  ...
  .controller('AppCtrl', function($scope, $ionicModal, $timeout, $http, $ionicSideMenuDelegate , $state,
     $rootScope, $ionicPopup, $translate, $cordovaDialogs, $cordovaBarcodeScanner, $ionicHistory, $ionicLoading) {
  ...
  if (localStorage.getItem("response") === null) {   
    localStorage.setItem('response','{"sold":[],"stock":[],"spent":[],"Account": 0 ,"deposit": [], "bank":"", "loan": 0 ,"payments":[],"institute":""}');
    var d=new Date();
 ...
 $scope.go_to_sold_detail = function (indexf) {
  ...
  $scope.addSale = function (saleAmt, saleReason) {   
      if(isNaN(saleAmt) || saleAmt<0){
          newerror("Enter the sale amount");
          return null;
      }
      var sresponse = localStorage.getItem('response');
      var replace = angular.fromJson(sresponse);
      var d = new Date();
      replace.sold.push({amount: saleAmt , reason: saleReason , year: d.getFullYear(), 
        month: d.getMonth(), day: d.getDay(), time: d.getTime()});
      localStorage.setItem('response', (angular.toJson(replace)));
      $rootScope.stock = replace.stock;
      $rootScope.sold = replace.sold;
      drefresh();
      ...
      { text: 'Stop' },
      {
        ...
        onTap: function(e) {
          if (!$scope.data.amount) {
             ...
             var d= new Date();
             $rootScope.spent.push({"onWhat":$scope.data.onWhat,"amount":$scope.data.amount, "date":d.time});
             saveMove()
           }
         }
       }
  ...
  $scope.saleservice=function(){
   $state.go('soldservice');
  }
  ...
  var show = function() {
    $ionicLoading.show({
      ...
      var dsales=function(){
       ...
       var dservice=function(){
         ...
         var amount=0;
         angular.forEach($rootScope.sold, function(value, key){          
           amount=amount + ($rootScope.sold[index].amount - 0) - 0;          
           index=index+1;
         });
         return (amount).toFixed(4);
     }
    ...
    $scope.serviceBalance=function(){
        var value= ($scope.dservice - $scope.dspent) - 0;
        if(value<0)
        {
          return 0;
        }
  ...
  var service = function()
   {
    ...
    angular.forEach($rootScope.sold,function(value,key){
     ...
     amount = amount + ($rootScope.sold[index].amount - 0) ;
      ...
      { text: 'Stop' },
      {
        ...
        onTap: function(e) {
          if (!$scope.data.amount) {
             ...
             var d= new Date();
             $rootScope.spent.push({"onWhat":$scope.data.onWhat,"amount":$scope.data.amount,"date":d.getTime()});
             saveMove();
           }
         }
       }
  ...
  $scope.nrloans = function(index){
   $cordovaDialogs.confirm('You are about to reset loans', 'Reset All Expenses', ['Yes','NO'])
   .then(function(buttonIndex) {
     // no button = 0, 'OK' = 1, 'Cancel' = 2
     if(buttonIndex==1){
       resetloans();
    ...
    $scope.nrbanks = function(index){
   $cordovaDialogs.confirm('You are about to reset banking', 'Reset All Expenses', ['Yes','NO'])
   .then(function(buttonIndex) {
     // no button = 0, 'OK' = 1, 'Cancel' = 2
     if(buttonIndex==1){
       resetbanking();
  ...
  .controller('Loans',function($scope,$ionicModal,$rootScope){
    ...
    .controller('bank', function($scope, $ionicModal, $rootScope) {

     var response = localStorage.getItem('response');

     $scope.institute = (angular.fromJson(response)).bank;
     $scope.amount = (angular.fromJson(response)).account;
     $scope.payments = (angular.fromJson(response)).deposit;
     ...
     $ionicModal.fromTemplateUrl('templates/addbank.html', {
        scope: $scope
     }).then(function(modal) {
        $scope.addmodal = modal;
    });

    $ionicModal.fromTemplateUrl('templates/editbank.html', {
      scope: $scope
    }).then(function(modal) {
      $scope.editinstitute = modal;
    });

    $ionicModal.fromTemplateUrl('templates/paybank.html', {
      scope: $scope
    }).then(function(modal) {
      $scope.paymodal = modal;
    });
    ...
    $scope.withdraw = function(amt) {
        if(!isNaN(amt)) {
         $scope.amount = ($scope.amount - 0 ) - (amt - 0);
         var d=new Date();
         $scope.payments.push({type:"withdrew",amount:amt,date:d.getTime()})
         $scope.addmodal.hide();
         saveMove();
       }
    }
    ...
    $scope.deposit = function(amt) {
        ...
        if($scope.amount>0){
          var d=new Date();
          $scope.payments.push({type:"deposited",amount:amt,date:d.getTime()})
          $scope.paymodal.hide();
    ...
    var saveMove = function() {
      var sresponse = localStorage.getItem('response');
      var replace = angular.fromJson(sresponse);
      replace.account = $scope.amount;
      replace.deposit=$scope.payments
      replace.bank=$scope.institute;
      localStorage.setItem('response',(angular.toJson(replace)));
    } 
